...
void test_hh_map_methods(void) {
  ...
  try {
    ...
    prev_b->c.bits = 0b01001;
    prev_b->tv.taken = 0b01001;
    //
    hash_ref->c.bits = 1;
    hash_ref->tv.taken = 1;
    //
    auto a = prev_b->c.bits;
    ...
    test_hh->place_back_tv.taken(hash_ref,hole,buffer,end);

    cout << "----" << endl;

    for ( int i = 0; i < 8; i++ ) {
      cout << "(" << i + 58   <<  ")" << bitset<32>(prev_b->c.bits) << "  " << bitset<32>(prev_b->tv.taken) << endl;
      prev_b++;
    }
    cout << "--- t2 ---" << endl;

    prev_b = prev_a - (prev_a->c.bits >> 1);
    cout << bitset<32>(prev_b->c.bits) << "  " << bitset<32>(prev_b->tv.taken) << endl;

    auto c = prev_b->c.bits & ~(0x1);
    offset = countr_zero(c);

    prev_a = prev_b + offset;
    cout << bitset<32>(prev_a->c.bits) << "  " << bitset<32>(prev_a->tv.taken) << endl;
    ...
    test_hh->remove_back_tv.taken(hash_ref,hole,buffer,end);
    for ( int i = 0; i < 8; i++ ) {
      cout << "(" << i + 58   <<  ")" << bitset<32>(prev_b->c.bits) << "  " << bitset<32>(prev_b->tv.taken) << endl;
      prev_b++;
    }
...
//     auto a = prev_b->c.bits;
//     auto b = prev_b->tv.taken;


//     cout << "1(" << (unsigned int)(prev_b - &test_buffer[0]) <<  ")\t" << bitset<32>(prev_b->c.bits) << " :: " << bitset<32>(prev_b->tv.taken) << endl;

//     hole = 3;
//     hh_element *vb_probe = prev_b + hole;
//     vb_probe->c.bits = (hole << 1);
//     vb_probe->tv.taken = (counter++ << 16);
...
//     test_hh->place_tv.taken(prev_b,hole,c,buffer,end);

//     // ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ----

//     cout << "2(" << (unsigned int)(prev_b - &test_buffer[0]) <<  ")\t" << bitset<32>(prev_b->c.bits) << " :: " << bitset<32>(prev_b->tv.taken) << endl;
...
//     test_hh->place_tv.taken(hash_ref,hole,c,buffer,end);
//     cout << "3(" << (unsigned int)(prev_b - &test_buffer[0]) <<  ")\t" << bitset<32>(prev_b->c.bits) << " :: " << bitset<32>(prev_b->tv.taken) << endl;
//     cout << "3(" << (unsigned int)(hash_ref - &test_buffer[0]) <<  ")\t" << bitset<32>(hash_ref->c.bits) << " :: " << bitset<32>(hash_ref->tv.taken) << endl;

//     cout << "CHECK offset and update" << endl;
//     auto w = prev_b->tv.taken;
//     cout <<  bitset<32>(w) << endl;
//     auto w_offset = get_b_offset_update(w);
//     cout << "1. w_offset: " << (int)w_offset << " " <<  bitset<32>(w) << endl;
//     cout << bitset<32>((prev_b + w_offset)->tv.taken) << endl;
//     cout << bitset<32>((prev_b + w_offset)->c.bits) << endl;
...
//     hash_ref->tv.taken = b;
//     c = a ^ b;
//     test_hh->tv.taken(hash_ref,hole,c,buffer,end);


//     cout << "4(" << (unsigned int)(hash_ref - &test_buffer[0]) <<  ")\t" << bitset<32>(hash_ref->c.bits) << " :: " << bitset<32>(hash_ref->tv.taken) << endl;
...
//     w_offset = get_b_offset_update(w);
//     cout << "1. w_offset: " << (int)w_offset << " " <<  bitset<32>(w) << endl;
//     cout << bitset<32>((hash_ref + w_offset)->tv.taken) << endl;
//     cout << bitset<32>((hash_ref + w_offset)->c.bits) << endl;
...
//     cout << "4(" << (unsigned int)(prev_b - &test_buffer[0]) <<  ")\t" << bitset<32>(prev_b->c.bits) << " :: " << bitset<32>(prev_b->tv.taken) << endl;
...
//     next_a->c.bits = 1;
//     next_a->tv.taken = 1;
//     test_hh->place_tv.taken(next_a,hole,c,buffer,end);
//     //
//     next_a_a->c.bits = 1;
//     next_a_a->tv.taken = 1;
//     test_hh->place_tv.taken(next_a_a,hole,c,buffer,end);
//     //
//     uint32_t value = 345;
//     uint32_t el_key = 4774;

//     test_hh->place_in_bucket_at_base(next_a_a_a,value, el_key);
//     test_hh->place_back_tv.taken(next_a_a_a, 0, buffer, end);

//     prev_b = hash_ref - 8;
//     for ( int i = 0; i < 32; i++ ) {
//       cout << "(" << (i + 56 - 64)  <<  ")\t" << bitset<32>(prev_b->c.bits) << " :: " << bitset<32>(prev_b->tv.taken) << endl;
//       prev_b++;
//     }

//     a = hash_ref->c.bits; // membership mask
//     b = hash_ref->tv.taken;
//       //
//     for ( int i = 0; i < 10; i++ ) {
//       hole = i*2;
...
//     test_hh->remove_back_tv.taken(hash_ref,2,buffer,end);
//     test_hh->remove_back_tv.taken(hash_ref,4,buffer,end);
//     test_hh->remove_back_tv.taken(hash_ref,6,buffer,end);
//     //
//     test_hh->remove_back_tv.taken(next_a,0,buffer,end);
//     test_hh->remove_back_tv.taken(next_a_a,0,buffer,end);
//     test_hh->remove_back_tv.taken(next_a_a_a,0,buffer,end);
//     //
...
//     /* test 3 */
//     hash_ref->c.bits = 0b00010101111100111101010111110111;
//     hash_ref->tv.taken = 0xFFFFFFFF;
//     //   
//     auto tester = (hash_ref + test_base_offset);  
//     tester->c.bits = ~(hash_ref->c.bits >> test_base_offset);
//     tester->tv.taken = 0xFFFFFFFF;

//     cout << "hash ref and test base: " << bitset<32>(hash_ref->c.bits) << " :: " << endl;
//     cout << "hash ref and test base: " << bitset<32>(tester->c.bits  << test_base_offset) << " :: " << endl;
//     cout << "hash ref and test base: " << bitset<32>(tester->c.bits) << " :: " << endl;

//     auto q = hash_ref->c.bits;
//     auto r = tester->c.bits;
//     cout << "hash ref and test base: " << bitset<32>((r << test_base_offset) & q) << " :: " <<  bitset<32>((r << test_base_offset) | q) << endl;


//     uint64_t B = hash_ref->c.bits;
//     uint64_t C = tester->c.bits;
//     uint64_t VV = C;
...
//     if ( offset < 32 ) {
//       //
//       (hash_ref + offset)->c.bits = 1;
//       (hash_ref + offset)->tv.taken = 0xFFFFFFFF;

//       cout <<  "COUNTR ONE FFFFFFFF: " << countr_one((hash_ref + offset)->tv.taken) << endl;
...
//       tmp = hash_ref + offset;
//       tmp->c.bits = offset << 1;
//       tmp->tv.taken = (counter-- << 18) | (1 << 16);
//     }


//     c = (hash_ref + test_base_offset)->c.bits;
//     offset = 0;
//     while ( c ) {
//       c = c & ~(0x1 << offset);
//       offset = countr_zero(c);
...
//       uint32_t backref = 0;
//       if ( !(tmp->c.bits & 1) ) {
//         backref = (tmp->c.bits >> 1);
//       }
//       cout << "(" << (i + 63 - 64) <<  ")\t" << bitset<32>(tmp->c.bits) << " :: " << bitset<32>(tmp->tv.taken) <<  " :: " << backref << endl;
//       tmp++;
//     }
...
//     auto a = hash_ref->c.bits;
//     auto b = hash_ref->tv.taken;
// 		c = a ^ b;

//     offset = 0;
//     hole = 32;
//     //
//     offset = 0;// test_hh->inner_bucket_time_swaps(hash_ref,hole,v_passed,time, buffer, end, 1);
...
//         backref = (tmp->c.bits >> 1);
//       }
//       cout << "(" << (i + 63 - 64) <<  ")\t" << bitset<32>(tmp->c.bits) << " :: "  << std::hex << tmp->tv.taken <<  " :: " << std::dec << backref << endl;
//       cout << "(" << (i + 63 - 64) <<  ")\t" << bitset<32>(tmp2->c.bits) << " :: " << std::hex << tmp2->tv.taken <<  " :: " << std::dec << backref << endl;
//       cout << "---" << endl;
//       tmp++;
//       tmp2++;
//     }
...
//     cout << "hash ref: " << bitset<32>(hash_ref->c.bits) << " " << bitset<32>(hash_ref->tv.taken)  << endl;
//     //
//     {
...
//       uint32_t a = hash_base->c.bits; // membership mask
//       uint8_t nxt = countr_one(a);
//       cout << "prev_ref: " << (hash_base+nxt)->tv.value << " " <<  bitset<32>((hash_base+nxt)->c.bits) <<  " nxt: " << (int)nxt  << endl;
//       nxt--;
//       hash_ref += nxt;
//       cout << "hash_ref: " << hash_ref->tv.value << " " <<  bitset<32>(hash_ref->c.bits) <<  " nxt: " << (int)nxt  << endl;
//       uint32_t b = hash_base->tv.taken;
//       //
...
//       test_hh->remove_bucket_tv.taken(hash_base,nxt_loc,a,b,buffer,end);
...
uint8_t test_usurp_membership_position(hh_element *hash_ref, uint32_t c_bits, hh_element *buffer,hh_element *end) {
  //
  uint8_t k = 0xFF & (c.bits >> 1);  // have stored the offsets to the bucket head
...
void test_some_bit_patterns_3(void) {   //  ----  ----  ----  ----  ----  ----  ----
...

  {
    ...
    base_probe->c.bits = 1;
    base_probe->tv.value = 1;

    (base_probe + 1)->c.bits = 1 << 1;


    base_probe_2->c.bits = 1;
    base_probe_2->tv.value = 5;
    ...
    cout << "offset_nxt_base = " << offset_nxt_base <<  " " << bitset<32>(c) <<  " "  << base_probe_2->tv.value << endl;
    ...
    uint32_t offset = 3;
       ...
       base_probe->c.bits =   0b0001011;
       base_probe_2->c.bits = 0b11101;
  base_probe->tv.taken = 0b1111111;
 base_probe_2->tv.taken = 0b00001111;

    //
    c = base_probe_2->c.bits;
    uint32_t time = 200;
    //
    base_probe_2 = base_probe;
    for ( int i = 0; i < 10; i++ ) {
      base_probe_2++;
      base_probe_2->tv.taken = --time;
      base_probe_2->tv.value = 10*i;
    }
    ...
    if ( min_probe != nullptr ) {
      cout << bitset<32>(min_base->c.bits) << " " << bitset<32>(min_base->tv.taken) << endl;
      cout << min_probe->tv.value  << " " << min_probe->tv.taken << endl;
    ...
    //
       ...
       base_probe_2->c.bits = 0b11101;
      base_probe->tv.taken =   0b1111111;
 base_probe_2->tv.taken = 0b1011111111;

    auto hash_ref = base_probe + 1;
    hash_ref->c.bits = (1 << 1);
    auto c.bits = hash_ref->c.bits;

    auto K = test_usurp_membership_position(hash_ref, c.bits,  buffer, end);
...
//         cout << "\tBUCKETS " << hex << p2.first << "  " <<  p2.second->tv.value << dec << endl;
//       }
//     }
...
//     cout << ba << endl;
//     if ( ba != nullptr ) {
//         cout << "BA " << hex <<  ba->tv.value << dec << endl;
//     }
...
//     for ( int i = 0; i < 8; i++ ) {
//       cout << std::hex << h1->tv.value << std::dec << "  --  " << bitset<32>(h1->c.bits)<< endl;
//       cout << std::hex << h2->tv.value << std::dec << "  --  " << bitset<32>(h2->c.bits)<< endl;
//       h1++; h2++;
//     }
...
//     if ( hel ) {
//       cout << "Give em: " << hex << hel->tv.value << dec << endl;
