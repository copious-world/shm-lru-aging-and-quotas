...
unsigned int hamming(bitset<64> &a,bitset<64> &b) {
    bitset<64> diff = a ^ b;
    return diff.count();
}

void flip_random_positions(bitset<64> &t_bits,uint8_t k,uint8_t start) {
    //
    random_device rd;  // a seed source for the random number engine
    mt19937 gen(rd()); // mersenne_twister_engine seeded with rd()
    uniform_int_distribution<uint8_t> pdist(start,63 + start);
    //
    bool taken[64];
...
int gen_vector_set() {
    ...
    uint8_t m = 8;
    //
    uint8_t kbits_to_flib = d/m;

    random_device rd;  // a seed source for the random number engine
    mt19937 gen(rd()); // mersenne_twister_engine seeded with rd()
    uniform_int_distribution<uint64_t> distrib(11, (((uint64_t)1 << 63 )- 13));
 
    // Use distrib to transform the random unsigned int
    // generated by gen into an int in [1, 6]
    for ( int n = 0; n != 10; ++n ) cout << distrib(gen) << ' ';
    cout << endl;

    bitset<64> current_B(0);
    current_B = distrib(gen);
    //
    cout << current_B << endl;
    deque<bitset<64>> HD_vectors;
    //
    ...
    for ( int i = 0; i < N; i++ ) {
        bitset<64> t_bits(0);
        flip_random_positions(t_bits,kbits_to_flib,i);
        current_B = current_B ^ t_bits;

        if ( used[current_B.to_ullong()] ) {
            i--; continue;
        }

        used[current_B.to_ullong()] = true;
        HD_vectors.push_back(current_B);
    }

    current_B = HD_vectors.at(0);
    for ( auto b : HD_vectors ) {
        cout << b << "::" << hamming(current_B,b) << endl;
    }
